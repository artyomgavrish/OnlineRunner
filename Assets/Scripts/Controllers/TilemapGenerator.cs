...
namespace PlatformerMVC
{
    ...
    {      
        ...
        private int _mapHeigh;
        private int _fillPercent;
        private int _factorSmoth;
        ...
        public TilemapGenerator(GeneratorLevelView levelView)
        {
            ...
            _mapHeigh = levelView.MapHeight;
            _fillPercent = levelView.FillPercent;
            _factorSmoth = levelView.FactorSmooth;
            _borders = levelView.Borders;

            _map = new int[_mapWidth,_mapHeigh];
        ...
        private void RandomFillMap()
        {
            ...
            for (int x = 0; x < _mapWidth; x++)
            {
                ...
                {
                    if( x == 0 || x == _mapWidth - 1 || y == 0 || y == _mapHeigh - 1)
                    {
                        if (_borders)
                        {
                            _map[x,y] = 1;
        ...
        private int GetWallCount(int x, int y)
        {
            ...
            for (int gridX = x - 1 ; gridX <= x+1; gridX++)
            {
                ...
                {
                    if (gridX >= 0 && gridX < _mapWidth && gridY >= 0 && gridY < _mapHeigh)
                    {
                        if (gridX != x || gridY != y)
                        {
        ...
        private void DrawTiles()
        {
            ...
            for (int x = 0; x <= _mapWidth; x++)
            {
                for (int y = 0; y <= _mapHeigh; y++)
                {
                    var positionTile = new Vector3Int(-_mapWidth/2 + x, -_mapHeigh/2 + y, 0);
